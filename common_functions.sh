	...
	function menu_consultas() {
	  clear;
	    while [ "$var" != 3 ]; do {
			    ...
			    1) Informaciócerca del propietario
			        2) Consulta sobre deudas 
				    3) Volver al menúncipal 
				        Ingrese su opcióeseada: ";
					    read var;
					        case $var in
							    1) ./propietario.sh;;
							        2) ./deudas.sh;;
								    3) echo -e '\e[0;32m Saliendo...\e[0m';sleep 1;clear;;
								        *) echo -e '\e[0;31m Opcióncorrecta\e[0m'
										    esac;
										      } done;
									      }
									      ...
									      function apartamento_es_valido () {
									        APT=$1; 
										  grep "^${APT}\:" propietarios.txt 2>/dev/null 1>/dev/null;
										    echo $?;
									    }
